...
import { WarehouseServices, type WarehouseErrorDetail } from "../../../services/warehouseServices";

interface WarehouseErrorDetailModalProps {
  isOpen: boolean;
  onClose: () => void;
  warehouseId: number | null;
}

export const WarehouseErrorDetailModal = ({ isOpen, onClose, warehouseId }: WarehouseErrorDetailModalProps): JSX.Element | null => {
  const [detail, setDetail] = useState<WarehouseErrorDetail | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      setIsLoading(true);
      try {
        const data = await WarehouseServices.getWarehouseDetail<WarehouseErrorDetail>(warehouseId, {
        ...
        console.error("Failed to fetch warehouse error details:", error);
      } finally {
        setIsLoading(false);
      }
    };
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-rose-50/50 border border-rose-100 rounded-xl p-4 flex items-center gap-4">
              <div className="p-3 bg-rose-100 rounded-lg text-rose-600">
                <Boxes className="w-6 h-6" />
              </div>
              <div>
                <p className="text-xs font-semibold text-rose-800 uppercase tracking-wider">Defective Products</p>
            ...
            <div className="bg-red-50/50 border border-red-100 rounded-xl p-4 flex items-center gap-4">
              ...
              <div>
                <p className="text-xs font-semibold text-red-800 uppercase tracking-wider">Total Defective Instances</p>
                <h3 className="text-2xl font-black text-red-950 mt-1">
                  {isLoading && !detail ? (
                    <Loader2 className="w-5 h-5 animate-spin text-red-600" />
                  ) : (
                    (detail?.summary.totalDefectiveInstances ?? 0).toLocaleString()
                  )}
                </h3>
              </div>
            </div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 bg-gray-50 p-3 rounded-lg border border-gray-200">
            <div className="relative w-80">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2 border border-gray-300 rounded-lg text-sm bg-white outline-none focus:ring-2 focus:ring-red-500"
              />
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <select
                ...
                className="border border-gray-300 rounded-lg text-sm p-1.5 bg-white outline-none focus:ring-2 focus:ring-red-500 cursor-pointer"
              >
            ...
            <table className="w-full text-left border-collapse">
              <thead className="bg-gray-50 text-[11px] uppercase text-gray-500 font-black border-b border-gray-200">
                <tr>
                  ...
                  <th className="p-4 text-right">Defective Qty</th>
                  ...
                  detail.data.map((item, index) => {
                    ...
                    return (
                      <Fragment key={item.productId}>
                        {/* Parent Row */}
                        <tr 
                          className={`hover:bg-red-50/20 transition-colors cursor-pointer ${isExpanded ? 'bg-red-50/10' : ''}`}
                        ...
                        >
                          ...
                          <td className="p-4 text-right font-black font-semibold font-mono text-red-600">{item.defectiveCount.toLocaleString()}</td>
                          <td className="p-4 text-center">
                            <button 
                              className="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-red-600 hover:text-red-800 hover:bg-red-50 rounded transition-all"
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            <td colSpan={6} className="p-0 bg-gray-50/50">
                              <div className="px-12 py-4 border-t border-b border-gray-100">
                                <h4 className="text-xs font-black uppercase text-gray-500 tracking-wider mb-2 flex items-center gap-1.5">
                                  <Layers className="w-3.5 h-3.5 text-red-500" /> Defective Batches breakdown
                               </h4>
                                  ...
                                  <table className="w-full text-left border-collapse">
                                    <thead className="bg-gray-100 text-[10px] uppercase text-gray-500 font-black border-b border-gray-150">
                                      <tr>
                                        ...
                                        <th className="p-2.5 text-right pr-4">Defective Qty</th>
                                      </tr>
                                    </thead>
                                      ...
                                      {item.batches && item.batches.length > 0 ? (
                                        item.batches.map(batch => (
                                          <tr key={batch.batchCode} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="p-2.5 text-gray-600 flex items-center gap-1">
                                              <Activity className="w-3.5 h-3.5 text-gray-400" /> {batch.productionLineName || "N/A"}
                                            </td>
                                            ...
                                            <td className="p-2.5 text-right font-bold text-red-600 pr-4">{batch.instanceCount.toLocaleString()} {item.unit}</td>
                                          </tr>
                                        ))
                                      ) : (
                                        <tr>
                                          <td colSpan={5} className="p-4 text-center text-gray-400 font-medium italic">
                                            No defective batches reported in this warehouse.
                                          </td>
                                        </tr>
                                      )}
                                    </tbody>
                  ...
                  !isLoading && (
                    <tr>
                      <td colSpan={6} className="p-12 text-center text-gray-400">
                        ...
                        No defective product inventory found in this warehouse.
                      </td>
                    </tr>
                  )
                )}
