...
export const approveRequest = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!warehouseId) {
            throw new Error("Target warehouseId is required for approval.");
        }
        const result = await MaterialRequestService.approveRequest(id, req.user!.employeeId, warehouseId);
